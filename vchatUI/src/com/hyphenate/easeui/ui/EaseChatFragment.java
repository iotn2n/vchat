...
import com.hyphenate.chat.EMMessageUtils;
...
import org.eclipse.paho.android.service.MessageEvent;
import org.eclipse.paho.android.util.MsgStatus;
...
public class EaseChatFragment extends EaseBaseFragment implements EMMessageListener {
    ...
    protected void setUpView() {
        ...
        if (chatType == EaseConstant.CHATTYPE_SINGLE) {
            ...
            if(EaseUserUtils.getUserInfo(toChatUsername) != null){
                ...
                toChatUserId = EaseUserUtils.getUserInfo(toChatUsername).getEid();
            }
        ...
        EMLog.i(TAG, "setUpView setRefreshLayoutListener ");
    ...
    protected void onConversationInit(){
        EMLog.i(TAG, " onConversationInit toChatUserId " + toChatUserId);
        ...
        if (resultCode == Activity.RESULT_OK) { 
            if (requestCode == REQUEST_CODE_CAMERA) { // capture new image
                ...
                if (locationAddress != null && !locationAddress.equals("")) {
                    ...
                    EMLog.i(TAG, "onActivityResult sendLocationMessage latitude ");
                } else {
    ...
    public void onMessageReceived(List<EMMessage> messages) {
        for (EMMessage message : messages) {
            ...
            EMLog.i(TAG, "onMessageReceived username " + username + " toChatUserId " + toChatUserId + " msgid " + message.getMsgId());
    ...
    public void onMessageChanged(EMMessage emMessage, Object change) {
        if(isMessageListInited) {
            messageList.refreshByMsgId(emMessage.getMsgId(), emMessage.status());
            messageList.refresh();
        }
    }
    ...
    protected void sendTextMessage(String content) {
        if(EaseAtMessageHelper.get().containsAtUsername(content)){
            ...
            EMLog.i(TAG, " sendTextMessage toChatUserId " + toChatUserId);
    ...
    protected void sendMessage(EMMessage message){
        ...
        message.setConversationId(toChatUserId);

        EMLog.i(TAG,"sendMessage message " + message.toString() + " toChatUserId "  + toChatUserId);
        //send message
    ...
    protected void emptyHistory() {
        ...
        new EaseAlertDialog(getActivity(),null, msg, null,new AlertDialogUser() {
            ...
            public void onResult(boolean confirmed, Bundle bundle) {
                if(confirmed){
                    EMClient.getInstance().chatManager().deleteConversation(toChatUserId, true);
                    messageList.refresh();
                }
            }
        }, true).show();;
    ...
    public void onUserEvent(MessageEvent event) {//参数必须是ClassEvent类型, 否则不会调用此方法
        if (event.topic.equals("/publish/ui/" + this.toChatUserId + "/status")) {
            EMLog.i(TAG, "onUserEvent msgId " + event.topic.toString() + " json " + event.jsobject.toString());
